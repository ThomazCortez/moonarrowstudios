...
$whereClause = "WHERE a.user_id IN (SELECT following_id FROM follows WHERE follower_id = :user_id) AND a.status = 'published'";
$params = [':user_id' => $current_user_id];

if (!empty($search)) {
    $whereClause .= " AND (a.title LIKE :search OR u.username LIKE :search)";
...
// Fetch assets from followed users
// Update your SQL query to include category name
$stmt = $pdo->prepare("
    SELECT SQL_CALC_FOUND_ROWS 
        ...
        (COALESCE(SUM(CASE WHEN av.vote_type = 'upvote' THEN 1 ELSE 0 END), 0) - 
         COALESCE(SUM(CASE WHEN av.vote_type = 'downvote' THEN 1 ELSE 0 END), 0)) AS score
    FROM assets a
...
$totalAssets = $totalStmt->fetchColumn();
...
<head>
    ...
    <title>Assets from Followed Users</title>
    <link href="https://cdn.quilljs.com/1.3.7/quill.snow.css" rel="stylesheet">
    <script src="https://cdn.quilljs.com/1.3.7/quill.min.js"></script>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/animate.css/4.1.1/animate.min.css" />
    <style>
        /* Consistent CSS from previous pages */
        :root {
...
<body>
    <div class="container">
        ...
        <div class="my-4 animate__animated animate__fadeIn">
            <h1>Assets from Users You Follow</h1>
            <p class="text-muted">Stay updated with assets from people you follow</p>
        </div>
        ...
        <form method="GET" class="d-flex align-items-center justify-content-between search-filter-container animate__animated animate__fadeIn">
            <div class="d-flex align-items-center flex-grow-1">
                <input type="text" name="search" class="form-control me-2 animate__animated animate__fadeInLeft" 
                       placeholder="Search assets or creators..." value="<?= htmlspecialchars($search) ?>">
                <select name="filter" class="form-select me-2 animate__animated animate__fadeInLeft">
                    ...
                    <option value="highest_score" <?= $filter == 'highest_score' ? 'selected' : '' ?>>Highest Score</option>
        ...
        <!-- Assets List -->
<div class="assets-list animate__animated animate__fadeInRight">
    <br>
        ...
        <?php if (!empty($assets)): ?>
            ...
            ?>
                <div class="card animate__animated animate__fadeInUp" style="animation-delay: <?= $animation_delay ?>s;">
                    <div class="card-body">
                        <h3 class="card-title">
                            <a href="view_asset.php?id=<?= $asset['asset_id'] ?>" class="text-decoration-none">
                                <?= htmlspecialchars($asset['title'] ?? 'Untitled Asset') ?>
                            </a>
                        </h3>
                        <p class="card-text text-light">
                            <em>Posted <?= formatTimeAgo($asset['created_at']) ?> by 
                                <a href="profile.php?id=<?= htmlspecialchars($asset['user_id']) ?>" 
            ...
            <?php else: ?>
                <div class="no-assets-message animate__animated animate__fadeIn">
                    <i class="bi bi-folder" style="font-size: 4rem; opacity: 0.5;"></i>
                    <h3>No Assets Found</h3>
                    <p>
                        <?php if (!empty($search)): ?>
                            No assets matching your search criteria were found.
                        <?php else: ?>
                            No assets from users you follow yet. Try following more creators!
                        <?php endif; ?>
                    </p>
                    <?php if (!empty($search)): ?>
