...
$whereClause = "WHERE p.user_id IN (SELECT following_id FROM follows WHERE follower_id = :user_id)";
$params = [':user_id' => $current_user_id];

if (!empty($search)) {
    // Add wildcard support for hashtags
    $hashtagSearch = str_replace('#', '', $search);
    $whereClause .= " AND (p.title LIKE :search 
                        OR p.content LIKE :search 
                        OR u.username LIKE :search 
                        OR p.hashtags LIKE :hashtag_search)";
    $params[':search'] = "%$search%";
    $params[':hashtag_search'] = "%$hashtagSearch%";
}
...
$stmt = $pdo->prepare("
    SELECT SQL_CALC_FOUND_ROWS p.id, p.title, p.content, p.created_at, p.upvotes, p.downvotes, 
           p.views, p.hashtags, u.username, u.user_id,
...
<head>
    ...
    <title>MoonArrow Studios - Following Posts</title>
    <style>
        :root {
...
<body>
    <div class="container">
        ...
        <div class="my-4 animate__animated animate__fadeIn">
            ...
            <p class="text-muted">Stay updated with posts from people you follow</p>
        </div>

        <!-- Search and Filter Section - REMOVED CONTAINER -->
        <form method="GET" class="d-flex align-items-center justify-content-between search-filter-container animate__animated animate__fadeIn">
            <div class="d-flex align-items-center flex-grow-1">
                <input type="text" name="search" class="form-control me-2 animate__animated animate__fadeInLeft" 
                       placeholder="Search posts, users, or content..." value="<?= isset($_GET['search']) ? htmlspecialchars($_GET['search']) : '' ?>">
        ...
        <!-- Post List - RESTRUCTURED TO MATCH REQUEST -->
        <div class="post-list animate__animated animate__fadeInRight">
            ...
            <div class="posts-container">
                <?php if (!empty($following_posts)): ?>
                    ...
                    foreach ($following_posts as $post): 
                        $post_count++;
                        $animation_delay = min($post_count * 0.15, 2);
                    ?>
                            ...
                            <div class="card-body">
                                ...
                                <p class="card-text text-light">
                                    <em>Posted on <?= $post['created_at'] ?> by 
                                        <a href="profile.php?id=<?= htmlspecialchars($post['user_id']) ?>" 
                                        class="text-decoration-none username-link"> <!-- Add this class -->
            ...
            <?php else: ?>
                <div class="no-posts-message animate__animated animate__fadeIn">
                    ...
                    <p>
                        ...
                        <?php else: ?>
                            No posts from users you follow yet. Try following some users to see their posts here!
                        <?php endif; ?>
                    </p>
                    <?php if (!empty($search)): ?>
